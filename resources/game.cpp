...
void Game::displayMainMenu(QString title,QString play)
{
  ...
  //Create the title
    ...
    Button * tableButton = new Button("Table",titleText);
    int txPos = 160;
    int tyPos = 230;
    tableButton->setPos(txPos,tyPos);
    connect(tableButton, SIGNAL(clicked()),this,SLOT(showTable()));
...
void Game::gameOver()
{
    ...
    userScore = QString::number(score->getScore());
    mytable->addRow(userName,userScore);
    mytable ->show();
